...
namespace IVLab.MinVR3
{
    ...
    public class MainPaintingAndReframingUI : MonoBehaviour
    {
        ...
        public Spline FindClosestSpline(List<Vector3> centers, out int bestIndex, out float splineStartIndex, out float splineEndIndex)
        {
            ...
            {
                knots.Add(new BezierKnot(c));
                // drawingSpline.Add(new BezierKnot(c));
                //knots.Add(new BezierKnot(new float3(x, y, z)));
            }
            drawingSpline.Knots = knots;
            // m_SplineColoredContainer.AddSpline(drawingSpline);
            // Debug.Log("DRAWING SPLINE: " + drawingSpline.EvaluatePosition(1));
            ...
            for (int index = 0; index < m_SplineContainer.Splines.Count; index++)
            {
                ...
                for (int i = 0; i < sampleCount; i++)
                {
                    ...
                    Vector3 p1 = drawingSpline.EvaluatePosition(t1);
                    Vector3 d1 = drawingSpline.EvaluateTangent(t1);
